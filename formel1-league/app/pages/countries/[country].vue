...
const countryName = computed(() => decodeURIComponent(route.params.country))
...
const countryTrackNames = computed(() =>
  Object.entries(trackInfo)
    .filter(([, info]) => info.country === countryName.value)
    .map(([name]) => name)
)

const continent = computed(() => countryContinent[countryName.value] ?? '')

const countryRaces = computed(() =>
  allResults.value.filter(r =>
    countryTrackNames.value.includes(processTrackName(r.Track).name)
  )
)

const trackCount = computed(() => {
  const names = new Set(countryRaces.value.map(r => processTrackName(r.Track).name))
  return names.size
})
...
const headerStats = computed(() => {
  ...
  return {
    totalRaces: countryRaces.value.length,
...
const notFound = computed(() => !loading.value && countryRaces.value.length === 0 && allResults.value.length > 0)
...
<template>
  <div class="bg-slate-200 min-h-screen">
    ...
    <div class="container mx-auto px-4 py-8">
      ...
      <p v-else-if="notFound" class="text-gray-600">Country "{{ countryName }}" not found.</p>

      <div v-else-if="countryRaces.length > 0">
        <!-- Header card -->
        <div class="bg-slate-800 rounded-lg p-6 mb-6 shadow-lg">
          <div class="flex items-baseline justify-between gap-4 mb-1">
            <h2 class="text-3xl font-bold text-white">{{ countryName }}</h2>
            <span v-if="continent" class="text-sm text-gray-400 flex-shrink-0">{{ continent }}</span>
          </div>
        ...
        <div class="flex flex-col sm:flex-row gap-6 mb-8">
            ...
            <!-- Top Drivers -->
            <div class="flex-1 bg-slate-800 rounded-lg shadow-lg overflow-hidden border border-slate-700">
        ...
        <div class="overflow-x-auto mb-8">
          <table class="bg-slate-800 rounded-lg overflow-hidden shadow-lg">
            ...
            <tbody class="divide-y divide-slate-700">
              <tr v-for="race in countryRaces" :key="race.id">
          ...
          <p v-if="countryRaces.some(r => r.Track?.includes('(Long)'))" class="text-[11px] text-gray-500 italic mt-2 px-1">* Alternate long layout</p>
        </div>
      </div>
    </div>
  </div>
